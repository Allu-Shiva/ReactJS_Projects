...
class AppStore extends Component {
  ...
  searchInput = event => {
    ...
    this.setState({userInput: textInput})
  }

  render() {
    const {activeTabId, userInput} = this.state
    let filteredApps = appsList.filter(
      eachApp => eachApp.category === activeTabId,
    )
    if (userInput !== '') {
      filteredApps = filteredApps.filter(eachItem =>
      ...
      <div className="bgContainer">
        ...
        <ul className="tabContainer">
          {tabsList.map(eachTab => (
            <TabItem
              Tab={eachTab}
              activeTabStatus={activeTabId === eachTab.tabId}
        ...
        <ul className="appContainer">
          {filteredApps.map(eachItem => (
            <AppItem app={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
