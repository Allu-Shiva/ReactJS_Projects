...
class Gallery extends Component {
  ...
  renderImage = imageObj => {
    const {imageUrl, imageAltText} = imageObj
    return <img src={imageUrl} alt={imageAltText} className="selectImage" />
  }
        ...
        <ul className="thumbnailContainer">
          {imagesList.map(eachImage => (
            <ThumbnailItem
              ...
              key={eachImage.id}
              activeStatus={activeImage.id === eachImage.id}
            />
          ))}
        </ul>
      </div>
